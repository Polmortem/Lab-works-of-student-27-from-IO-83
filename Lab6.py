...
def generate_matrix():
    def f(X1, X2, X3):
        y = 6.7 + 2.0 * X1 + 2.4 * X2 + 1.6 * X3 + 5.7 * X1 * X1 + 0.7 * X2 * X2 + 2.6 * X3 * X3 + 8.7 * X1 * X2 + 0.8 * X1 * X3 + 3.1 * X2 * X3 + 1.3 * X1 * X2 * X3 + randrange(0, 10) - 5
        return y
...
def average_value(arr, orientation):
    average = []
    if orientation == 1:
        for rows in range(len(arr)): average.append(sum(arr[rows]) / len(arr[rows]))
    else:
        for column in range(len(arr[0])):
            number_arr = []
            for rows in range(len(arr)): number_arr.append(arr[rows][column])
...
def a(first, second):
    necessary_a = 0
    for j in range(N): necessary_a += matrix_x[j][first - 1] * matrix_x[j][second - 1] / N
    return necessary_a


def find_known(number):
    necessary_a = 0
    for j in range(N): necessary_a += middle_y[j] * matrix_x[j][number - 1] / 15
    return necessary_a


def check_result(arr_b, k):
    y_i = arr_b[0] + arr_b[1] * matrix[k][0] + arr_b[2] * matrix[k][1] + arr_b[3] * matrix[k][2] + arr_b[4] * matrix[k][3] + arr_b[5] * matrix[k][4] + arr_b[6] * matrix[k][5] + arr_b[7] * matrix[k][6] + arr_b[8] * matrix[k][7] + arr_b[9] * matrix[k][8] + arr_b[10] * matrix[k][9]
    return y_i
...
for i in range(len(matrix_x)):
    if i < 8:
        x_1 = x1[0] if matrix_pe[i][0] == -1 else x1[1]
        x_2 = x2[0] if matrix_pe[i][1] == -1 else x2[1]
        x_3 = x3[0] if matrix_pe[i][2] == -1 else x3[1]
    else:
...
known_values = [my, find_known(1), find_known(2), find_known(3), find_known(4), find_known(5), find_known(6), find_known(7), find_known(8), find_known(9), find_known(10)]

beta = solve(values, known_values)
print("\nРівняння регресії:")
print("{:.3f} + {:.3f} * X1 + {:.3f} * X2 + {:.3f} * X3 + {:.3f} * Х1X2 + {:.3f} * Х1X3 + {:.3f} * Х2X3 + {:.3f} * Х1Х2X3 + {:.3f} * X11^2 + {:.3f} * X22^2 + {:.3f} * X33^2 = ŷ\n\nПеревірка".format(beta[0], beta[1], beta[2], beta[3], beta[4], beta[5], beta[6], beta[7], beta[8], beta[9], beta[10]))
for i in range(N): print("ŷ{} = {:.3f} ≈ {:.3f}".format((i + 1), check_result(beta, i), middle_y[i]))

while not homogeneity:
    print("\n" + " " * 65 + "Матриця планування" + " " * 65 + "      X1           X2           X3          X1X2        X1X3         X2X3         X1X2X3       X1X1         X2X2         X3X3          Yi ...")
    for row in range(N):
        print(" ", end=' ')
        for column in range(len(matrix[0])): print("{:^12.3f}".format(matrix[row][column]), end=' ')
        print(" ")
    print("\n")
    dispersion_y = [0.0 for x in range(N)]
    for i in range(N):
        dispersion_i = 0
        for j in range(m): dispersion_i += (matrix_y[i][j] - middle_y[i]) ** 2
    ...
    if Gt > Gp and m < 25:
        ...
        print("Дисперсія не є однорідною. Повторна спроба при m += 1...")
        m += 1
    if m == 25: exit()
...
print("Рівняння регресії за критерієм Стьюдента\n{:.3f} + {:.3f} * X1 + {:.3f} * X2 + {:.3f} * X3 + {:.3f} * Х1X2 + {:.3f} * Х1X3 + {:.3f} * Х2X3 + {:.3f} * Х1Х2X3 + {:.3f} * X11^2 + {:.3f} * X22^2 + {:.3f} * X33^2 = ŷ\n\nПеревірка".format(student_arr[0], student_arr[1], student_arr[2], student_arr[3], student_arr[4], student_arr[5], student_arr[6], student_arr[7], student_arr[8], student_arr[9], student_arr[10]))
for i in range(N): print("ŷ{} = {:.3f} ≈ {:.3f}".format((i + 1), check_result(student_arr, i), middle_y[i]))
d = 11 - student_arr.count(0)
dependent += N - d
independent += d
print('Математична модель адекватна експериментальним даним.') if fischer_test() else print('Математична модель не є адекватною експериментальним даним.')
